<template>
  <div class="light" v-if="tableData">
    ...
    <el-button
      class="edit"
      type="primary"
      @click="(dialogFormVisible = true), addchangeLight()"
      >{{ addTitle }}</el-button
    >
    ...
    >
      ...
      <el-table-column prop="idOpen" label="操作" width="300" align="center">
        <template slot-scope="scope">
          <el-switch
...
export default {
  ...
  methods: {
    ...
    change(id, isOPen) {
      console.log(12);
      this.PostAxios("isOpenLightMsg", { isOPen, id }).then((res) => {
        console.log(id, isOPen, res.data);
        if (res.data.code == "40010" && isOPen == "on") {
          this.$message.success("路灯已开启");
        }
        if (res.data.code == "40010" && isOPen == "off") {
          this.$message.success("路灯已关闭");
        }
      });
    },
    getLightData() {
      this.GetAxios("lightMsg").then((res) => {
    ...
    addLight() {
      this.PostJsonAxios("addLightMsg", this.form).then((res) => {
        if (res.data.flag == 1) {
          this.$message.success("新的路灯已添加成功");
          this.getLightData();
        } else {
    ...
    editLight(row) {
      ...
      this.PostAxios("idLightMsg", params).then((res) => {
    ...
    editLigeing() {
      this.PostJsonAxios("editLightMsg", this.form).then((res) => {
        if (res.data.flag == 1) {
          this.$message.success("修改路灯信息成功");
          this.getLightData();
        } else {
          this.$message.error("修改路灯信息失败");
        }
      });
    },
    deleteLight(id) {
      console.log(id);
      this.PostAxios("deleteLightMsg", { id }).then((res) => {
        console.log(res);
        if (res.data.flag == 1) {
          this.$message.success("删除该路灯成功");
          this.getLightData();
        } else {
          this.$message.success("删除该路灯失败");
        }
      });
    },
  },
